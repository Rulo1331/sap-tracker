...
# Cambiamos el título y el ícono
st.set_page_config(page_title="Reporte Seguim. Serv. y Otros", page_icon="📑")
...
# IMPORTAMOS LA NUEVA FUNCIÓN QUE CREAMOS EN SCRAPER.PY
from scraper import get_multiple_report3_exports, read_report_txt  # noqa: E402

st.title("📑 Reporte Seguim. Serv. y Otros")

st.markdown(
    "Ingresa números de solicitud de pedido para consultar el "
    "**Reporte seguim. serv. y otros** (se exporta como texto con tabuladores)."
)
...
with tab1:
    text_input = st.text_area(
        ...
        key="report3_text_input", # Clave única para evitar conflictos con la otra página
    )
    if text_input:
...
with tab2:
    uploaded_file = st.file_uploader(
        "Sube tu archivo Excel", type=["xlsx", "xls"], key="report3_file_uploader"
    )
    if uploaded_file:
...
if st.button("🔍 Consultar reporte de servicios", disabled=not order_numbers, key="report3_button"):
    with st.spinner(f"Exportando {len(order_numbers)} reporte(s) desde SAP..."):
        # LLAMAMOS A LA TERCERA TRANSACCIÓN
        export_results = get_multiple_report3_exports(order_numbers)
                ...
                if r.html_path and Path(r.html_path).exists():
                    with open(r.html_path, "rb") as f:
                        st.download_button(
                            ...
                            key=f"html_report3_{r.order_number}",
                        )
                continue
            try:
                # Utilizamos la misma función de lectura porque el formato es el mismo (TXT)
                ...
                with open(r.file_path, "rb") as f:
                    st.download_button(
                        ...
                        key=f"dl_report3_{r.order_number}",
                    )
